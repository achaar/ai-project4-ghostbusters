...
class DoubleInferenceAgent(bustersAgents.BustersAgent):
    ...
    def getAction(self, gameState):
        ...
        for index,inf in enumerate(self.inferenceModules):
            ...
            if self.observe:
                inf.observe(gameState)
            self.ghostBeliefs[index] = inf.getBeliefDistribution()
            # print("Ankur: ",self.ghostBeliefs[index])
            # print("REF: ", dists[index])
            # self.distCompare(self.ghostBeliefs[index], dists[index])
            if self.numMoves >= self.burnIn: self.distCompare(self.ghostBeliefs[index], dists[index])  #print("ERROR!")
        self.display.updateDistributions(self.ghostBeliefs)
        random.seed(self.seed + self.numMoves)
        return action
    ...
    def distCompare(self, dist, refDist):
        ...
        # print("Student keys: ",dist.keys())
        # print("Ref Keys: ", refDist.keys())
        ...
        if l2 > self.L2Tolerance:
            if self.errors == 0:
                ...
                # print("Student: ",dist.items())
                # print("Reference: ", refDist.items())
